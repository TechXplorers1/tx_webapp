...
import { getDatabase, push, goOffline, goOnline } from "firebase/database"; // Import Firebase functions
...
const ManagerWorkSheet = () => {
    ...
    if (!managerFirebaseKey) return;
      ...
      const unsubscribeUsers = onValue(usersQuery, (snapshot) => {
        ...
        const employees = Object.entries(usersData)
          .filter(([_, user]) => user.roles?.includes('employee'))
          .map(([key, user]) => ({
            firebaseKey: key,
            ...user,
  ...
  return () => {
    console.log("ManagerWorksheet unmounted. Cleaning up idle timer.");
    // Clear the timer
    if (idleTimer) {
      clearTimeout(idleTimer);
    }
