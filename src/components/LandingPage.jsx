...
const LandingPage = () => {
  ...
  const handleBookServiceClick = (item) => {
    ...
    // If the user isn't logged in or has no service data, return an empty array
    if (!isLoggedIn || !user || !user.serviceRegistrations) {
      return [];
    }
    // Extract the names of all registered services into an array
    return Object.values(user.serviceRegistrations).map(reg => reg.service);
  }, [user, isLoggedIn]);
              ...
              <Carousel.Item key={item.id} className="carousel-item-custom position-relative">
                <div className="carousel-overlay-content position-absolute top-50 start-50 translate-middle text-center text-white">
                  <h3>{item.text}</h3>
                    ...
                    {(() => {
                      // Check if the current service in the carousel is in the user's active list
                      const isServiceActive = activeServices.includes(item.service) || (item.isJobSupport && activeServices.includes('Job Supporting'));

                      if (isServiceActive) {
                        // If the service is active, show the "Your Dashboard" button
                        return (
                          <Link to="/clientdashboard" className="header-action-btn btn-book">
                            Your Dashboard
                          </Link>
                        );
                      } else {
                        // If the service is not active, show the "Book a Service" button
                        return (
                          <div
                            onClick={() => handleBookServiceClick(item)}
                            className="header-action-btn btn-book"
                    ...
                    <Link to={item.path} className="header-action-btn">
                      Learn More
                    </Link>
                  </div>
                </div>
